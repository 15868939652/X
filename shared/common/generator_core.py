...
from common.word_bank import expand as expand_word_banks
from common.industry_kb import search as search_knowledge
...
from constants import MODE_NAMES, MODE_WEIGHTS, MODES, SEGMENTED_MODES
...
def _parse_one_shot_output(raw: str) -> tuple[str, str]:
    ...
    _, _, article = parse_segmented_output(raw)
...
def _layer_knowledge(keyword: str) -> str:
    """Build domain-knowledge prompt fragment from the industry KB.

    Returns empty string when no KB entry matches *keyword*, so existing
    prompt identity is preserved for unmatched keywords.
    """
    parts = []
    for cat in ("hospitals", "checkups", "symptoms"):
        items = search_knowledge(cat, keyword, n=2)
        if items:
            parts.extend(items)
    if not parts:
        return ""
    return "\n\n【领域知识（仅用于保证事实准确，不可直接大段复制）】\n" + "\n".join(parts)


def _layer_core(mode: str, keyword: str) -> str:
    prompt_file = MODES[mode][0]
    base = _load(prompt_file)
    base = expand_word_banks(base)
...
def _build_prompt(keyword: str, mode: str, profile: str, style: str, trigger: str, platform: str, platform_prompt: str, dynamics: dict) -> tuple[str, bool]:
    ...
    knowledge = _layer_knowledge(keyword)
    dynamic_text = (
        "【随机因子层】\n"
...
def _record_stage(record: dict, name: str, t_start: float) -> None:
    t_end = time.time()
    record["stages"][name] = {
        "start": datetime.fromtimestamp(t_start).isoformat(timespec="milliseconds"),
        "end": datetime.fromtimestamp(t_end).isoformat(timespec="milliseconds"),
...
def generate_article(
    ...
    _record_stage(record, "prompt_build", t_stage)

    if generation_mode == "one_shot":
        title, article, record = _generate_one_shot_article(
            keyword,
            platform,
            platform_prompt,
            provider,
        ...
        _record_stage(record, "total", t_start)
        return title, article, record
    ...
    with step("[1/3] 生成初稿 + 标题"):
        ...
        if gen_result.empty:
            ...
            raise GenerationTaskError(record["failure_reason"], record)
    _record_stage(record, "draft_generation", t_stage)

    title, segments, article = parse_segmented_output(raw)
    ...
    _record_stage(record, "first_scoring", t_stage)
    ...
    if score < min_score:
        ...
        with step(f"  robust rewrite [{','.join(targets) if segments else 'whole'}]"):
            try:
                if segments and targets:
                    ...
                    article = join_segments(segments)
                else:
        ...
        eval_result = _rule_only_eval(article, platform=platform, mode=mode)
        _record_stage(record, "rewrite", t_stage)
