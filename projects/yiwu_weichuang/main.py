...
def run(platform_only: str = "", custom_count: int = 0):
    ...
    cat_weights = []
    cat_lists = []
    if selected_gyn: cat_weights.append(0.8); cat_lists.append(selected_gyn)
    if selected_and: cat_weights.append(0.1); cat_lists.append(selected_and)
    if selected_exam: cat_weights.append(0.1); cat_lists.append(selected_exam)
    if not cat_lists:
        cat_weights = [1.0]; cat_lists = [selected_gyn]

    total_w = sum(cat_weights)
    selected_kw = []
    for lst, w in zip(cat_lists, cat_weights):
        n = max(1, int(task_total * w / total_w))
        selected_kw.extend(lst[:n] if len(lst) >= n else lst[:])
    selected_kw = selected_kw[:task_total] if selected_kw else gyn_keywords[:task_total]
